...
export default function PopupBannerPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['banners', 'popup', { page, search, statusFilter }],
    queryFn: () => bannersApi.getAll({ type: 'popup', page, limit: 10, search, status: statusFilter }),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['banners', 'popup'] });
    },
  ...
  const columns: ColumnsType<Banner> = [
    ...
    {
      title: '노출 조건', dataIndex: 'displayCondition', key: 'displayCondition', width: 120,
      render: (v) => (
        <Tag color={v === 'once' ? 'orange' : 'default'}>
          {v === 'once' ? '하루 한 번' : '항상 표시'}
        </Tag>
      ),
    },
  ...
  return (
    <>
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Title level={4} style={{ margin: 0 }}>팝업 배너</Title>
